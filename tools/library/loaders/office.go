...
func (loader Office) loadDocx() ([]schema.Document, error) {
	...
	for _, file := range zipReader.File {
		if file.Name == "word/document.xml" {
			...
			textContent = loader.extractTextFromWordXML(content)
			break
		}
	}
...
func (loader Office) loadPptx() ([]schema.Document, error) {
	...
	for _, file := range zipReader.File {
		if slideFilePattern.MatchString(file.Name) {
			...
			slideText := loader.extractTextFromPowerPointXML(content)
			if slideText != "" {
				if allText.Len() > 0 {
...
func (loader Office) extractTextFromWordXML(xmlContent []byte) string {
...
func (loader Office) extractTextFromPowerPointXML(xmlContent []byte) string {
